@php
    /** @var mixed $media */
    $media = $media ?? null;
    $urlResolver = $urlResolver ?? null; // callable|null — fn($media): string
    ...
    // Allow caller to override the URL (e.g. via a proxy that serves with Content-Disposition: inline)
    $resolvedUrl = $urlResolver
        ? $urlResolver($media)
